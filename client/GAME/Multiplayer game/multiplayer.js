...
const state = { mode:"bot",difficulty:"easy",players:[],turn:1,busy:false,target:null,quiz:null,timer:null,lobbyTimers:[],stats:{captured:0,correct:0,duels:0,bases:0} };
...
function initPlayers(){
    ...
    state.players=names.map((name,id)=>({id,name,color:CONFIG.colors[id],alive:true,isHuman:id===0,baseRegion:null}));
}
...
function startGame(){
    clearLobby(); initPlayers(); resetMap(); state.turn=1;state.busy=false;state.stats={captured:0,correct:0,duels:0,bases:0};
    $("activityFeed").innerHTML=""; $("matchLabel").textContent=state.mode==="people"?"Phòng 4 người":"Đấu với máy · "+({easy:"Dễ",medium:"Vừa",hard:"Khó"}[state.difficulty]);
    showScreen("game"); addFeed(`<strong>${currentName()}</strong> đã dựng căn cứ tại ${region(state.players[0].baseRegion).name}.`,"good"); renderAll();
}

function renderAll(){ renderMap();renderPlayers();$("turnNumber").textContent=state.turn; const alive=state.players.filter(p=>p.alive).length;$("aliveCount").textContent=`${alive} còn lại`;$("missionProgress").style.width=`${state.stats.bases/3*100}%`; }
...
function makeQuestion(){
    const pool=elementPool();const e=random(pool);const type=Math.floor(Math.random()*5);let text,answer,kind;
    if(type===0){text=`Kí hiệu hóa học của nguyên tố ${e.name} là gì?`;answer=e.symbol;kind="Nhận diện kí hiệu";}
    else if(type===1){text=`Nguyên tố nào có số hiệu nguyên tử ${e.number}?`;answer=e.name;kind="Số hiệu nguyên tử";}
    else if(type===2){text=`Số hiệu nguyên tử của ${e.symbol} là bao nhiêu?`;answer=String(e.number);kind="Cấu tạo nguyên tử";}
    else if(type===3){text=`Nguyên tố ${e.symbol} nằm ở chu kì nào?`;answer=String(e.period);kind="Vị trí trong bảng";}
    else {text=`Nguyên tố ${e.symbol} thuộc nhóm nào?`;answer=String(e.group);kind="Vị trí trong bảng";}
    let distractors;
    if(type===0)distractors=shuffle(pool.filter(x=>x!==e).map(x=>x.symbol));
    else if(type===1)distractors=shuffle(pool.filter(x=>x!==e).map(x=>x.name));
    else if(type===2)distractors=shuffle(pool.filter(x=>x!==e).map(x=>String(x.number)));
    else if(type===3)distractors=shuffle(["1","2","3","4","5","6","7"].filter(x=>x!==answer));
    else distractors=shuffle(Array.from({length:18},(_,i)=>String(i+1)).filter(x=>x!==answer));
    return {text,answer,kind,options:shuffle([answer,...Array.from(new Set(distractors)).slice(0,3)])};
}
...
function resolveAttack(q,success){
    ...
    if(success){
        ...
        else {target.owner=q.attackerId;if(q.attackerId===0)state.stats.captured++;addFeed(`<strong>${attacker.name}</strong> chiếm ${target.name}.`,q.attackerId===0?"good":"");}
...
function eliminatePlayer(loserId,winnerId){
    const loser=state.players[loserId];loser.alive=false;REGIONS.filter(r=>r.owner===loserId).forEach(r=>{r.owner=winnerId;r.baseOwner=null;r.baseHp=0;});
    if(winnerId===0)state.stats.bases++;addFeed(`<strong>${loser.name}</strong> đã mất căn cứ và bị loại!`,"danger");
}
function runBotTurns(){
    const bots=state.players.filter(p=>p.alive&&!p.isHuman);let index=0;const next=()=>{if(index>=bots.length){state.turn++;state.busy=false;renderAll();setStatus("Lượt của bạn","Chọn một vùng sáng để tiếp tục chinh phục.");return;}const bot=bots[index++];setTimeout(()=>{botTurn(bot.id);renderAll();if(checkEnd())return;next();},500);};next();
}
function botTurn(id){
    ...
    if(defender===null){target.owner=id;addFeed(`<strong>${state.players[id].name}</strong> chiếm ${target.name}.`);return;}
    ...
    if(target.baseOwner===defender){target.baseHp--;addFeed(`<strong>${state.players[id].name}</strong> đánh trúng căn cứ ${state.players[defender].name}!`,"danger");if(target.baseHp<=0)eliminatePlayer(defender,id);}else{target.owner=id;addFeed(`<strong>${state.players[id].name}</strong> chiếm ${target.name}.`);}
}
function checkEnd(){
    const human=state.players[0],alive=state.players.filter(p=>p.alive);if(human.alive&&alive.length>1)return false;state.busy=true;renderAll();const won=human.alive&&alive.length===1;
    $("resultIcon").textContent=won?"🏆":"⚑";$("resultTitle").textContent=won?"Bạn đã thống nhất thế giới!":"Căn cứ của bạn đã thất thủ";$("resultText").textContent=won?"Kiến thức nguyên tố và chiến thuật của bạn đã đánh bại mọi đối thủ.":`${alive[0]?.name||"Đối thủ"} là người sống sót cuối cùng.`;
    $("resultStats").innerHTML=`<div class="result-stat"><strong>${state.stats.captured}</strong><span>Vùng đã chiếm</span></div><div class="result-stat"><strong>${state.stats.correct}</strong><span>Câu trả lời đúng</span></div><div class="result-stat"><strong>${state.stats.bases}/3</strong><span>Căn cứ phá hủy</span></div>`;setTimeout(()=>$("resultModal").classList.remove("hidden"),700);return true;
}
...
function returnSetup(){clearLobby();clearInterval(state.timer);$("resultModal").classList.add("hidden");showScreen("setup");}

document.addEventListener("DOMContentLoaded",()=>{
    createParticles();$("peopleModeBtn").addEventListener("click",startPeopleLobby);$("botModeBtn").addEventListener("click",()=>{state.mode="bot";startGame();});$("cancelLobbyBtn").addEventListener("click",returnSetup);
    document.querySelectorAll(".segment").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll(".segment").forEach(x=>x.classList.remove("active"));b.classList.add("active");state.difficulty=b.dataset.difficulty;}));
